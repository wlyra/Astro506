...
\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/bulk_viscosity.png}}
  \end{center}
  \caption[]{Bulk viscosity (second visosity or volume viscosity) is a
    resistance to compression / expansion that exists in liquids and
    non-ideal gases. Microphysically, it arises from the finite time
    it takes the energy of an expansion to be transferred to
    rovibrational modes. During this relaxation time, the system is in
    non-equilibrium and can dissipate energy. In the momentum
    equation, bulk viscosity behaves like extra pressure, but in the
    energy equation it behaves like viscosity, leading to
    heating. Shock viscosity is an artificial viscosity added to
    smooth discontinuities, that draws inspiration from this physical
    process.}
  \label{fig:bulk-viscosity}
\end{figure}
...
\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/shockviscosity-diffres.png}}
  \end{center}
  \caption[]{Function profile after one revolution using shock
    viscosity at low (128, left), and high (1024, right),
    resolution. The front discontinuity is more smoothed than the back
    discontinuity. At low resolution the diffusion is too strong, and
    yet the shock diffusion does not prevent Gibbs phenomena.}
  \label{fig:shockviscosity-smoothing}
\end{figure}
...
\begin{figure}
  \begin{center}
    \resizebox{.5\textwidth}{!}{\includegraphics{./figs/donor.png}}
  \end{center}
  \caption[]{Donor cell}
  \label{fig:donor}
\end{figure}
...
\begin{figure}
  \begin{center}
    \resizebox{.6\textwidth}{!}{\includegraphics{./figs/donor-advection.png}}
  \end{center}
  \caption[]{Donor cell advection}
  \label{fig:donor-advection}
\end{figure}
...
constant within each cell \figp{fig:donor-advection}. We then let the material flow through the cell interfaces, from left to
...
but, as the upstream differencing method, it is very diffusive \figp{fig:donor}.
...
\begin{figure}
  \begin{center}
    \resizebox{.6\textwidth}{!}{\includegraphics{./figs/laxwendroff_slope.png}}
  \end{center}
  \caption[]{Lax-Wendroff slope}
  \label{fig:laxwendroff_slope}
\end{figure}
