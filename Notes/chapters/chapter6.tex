...
The condition $C \leq 1$ means that $v\Delta t \leq \Delta x$. So,
qualitatevely, it means that the timestep must be such that no flux
crosses more than one grid cell at a time \figp{fig:cfl_condition},
which guarantees causality. This is a necessary, but not sufficient, intuitive condition for stability.


\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/cfl_condition.png}}
  \end{center}
  \caption[]{The Courant-Friedrichs-Lewy (CFL) condition. $u\Delta t$
    is the distance a signal travels within a timestep. This distance
    cannot be bigger than a grid cell ($C=u\Delta t/\Delta x < 1$, in
    blue), since cell has to know what happened upwind. A signal cannot
    jump grid cells ( $C >$ 1 in red), or causality is violated.}
  \label{fig:cfl_condition}
\end{figure}
...
Let us start simple, the two-body problem \figp{fig:two-body} 

\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/two-body.png}}
  \end{center}
  \caption[]{Setup for two-body problem.}
  \label{fig:two-body}
\end{figure}
...
over time the error accumulates, and becomes linear   \figp{fig:euler-error}. If $x\propto
...
\begin{figure}
  ...
  \end{center}
  \caption[]{Euler method. Advance to $t_{i+1}$ by using the derivative
    at $t_i$.}
...
\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/euler_error_growth.png}}
  \end{center}
  \caption[]{Error in Euler timestepping. The truncation error (left) is of
    order $\mathcal{O}(\Delta t^2)$, the accumulated error (right) is
...
leapfrog in fact achieves 4th order accuracy \figp{fig:kdk-error-growth}. Consider $i=0$
...
comparing \eq{eq:leap1} and \eq{eq:leap2}, the truncation error is
$\mathcal{O}\left(\Delta t^4\right)$.

\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/KDK_error_growth.png}}
  \end{center}
  \caption[]{The kick-drift-kick method has local truncation error of
    fourth order (left). Unlike Euler, where the error grows in time, the error in
KDK is bounded. Because of the high-order, machine
precision is quickly reached as the timestep decreases (right).}
  \label{fig:kdk-error-growth}
\end{figure}
...
The time reversibility of KDK leads to the property that the energy
error is bound \figp{fig:euler-leapfrog}. This is because a time-symmetric code cannot spiral out, since such
a motion would increase the size of the orbit. If an algorithm lets an orbit
grow in one direction in time, it lets the orbit grow when applied to the other
direction in time as well, as so it would not be time symmetric. This energy
conservation property makes KDK is a powerful method for gravitational problems (although there is phase error).

\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/euler_leapfrog_energy_bound.png}}
  \end{center}
  \caption[]{The leapfrog algorithm is time-reversible, which leads to the property that the energy error is bounded.
The time-reversibility is because it if you start from the future and
set $\Delta t \rightarrow -\Delta t$, you trace back the same steps to reach the
past. The Euler algorithm does not have this property.}
  \label{fig:euler-leapfrog}
\end{figure}


A graphical summary of the schemes studied so far is presented in
shown in \fig{fig:schemes-graphical}.

\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/schemes_graphical.png}}
  \end{center}
  \caption[]{Graphical representation of the schemes.}
  \label{fig:schemes-graphical}
\end{figure}
...
It advances by using 4 points in space (beginning end and two
intermmediate) and three in time (beginning, middle point, and end)   \figp{fig:rk4-graphical}
...
\begin{figure}
  \begin{center}
    \resizebox{\textwidth}{!}{\includegraphics{./figs/rk4_graphical.png}}
  \end{center}
  \caption[]{Graphical representation of the RK4 method. The
    derivatives at four points are used to pass from $P_1=(t_0, y_0$) to
   $(t_1,y_1)$, with different weights. First an euler step is used
    to pass to the middle point in $t$, using the derivative $k_1$  at
    $P_1$, arriving at the point $P_2$, which is the middle point in t
    and where the tangent $k_1$ lands it on the $y$ axis. The
    derivative $k_2$ at $P_2$ is computed, and used to find the point
    $P_3$, which is going back to $P_1$ and doing an Euler step to
    the middle point with $k_2$.  The point $P_4$ is found by Euler
    stepping from $P_1$ with the derivative $k_3$ at $P_3$, until the
    time $t_1$. The derivative at this point, $k_4$, is
    computed. Twice the weight is given to $k_2$ and $k_3$ than to $k_1$ and
    $k_4$.}
  \label{fig:rk4-graphical}
\end{figure}
...
To make the two expressions, \eq{eq:expand2} and \eq{eq:taylor} match, we need
